...
Usage: tools/replay_check.py --case CASE --first R --prefix N --path FILE [--lift APTN0] [--down] [--extra K] [--verbose]
...
def main():
    args = sys.argv[1:]
    opt = dict(lift=None, down=False, extra=0, verbose=False)
    i = 0
    while i < len(args):
        a = args[i]
        if a in ("--case", "--path"): opt[a[2:]] = args[i + 1]; i += 2
        elif a in ("--first", "--prefix", "--lift", "--extra"): opt[a[2:]] = int(args[i + 1]); i += 2
        elif a == "--down": opt["down"] = True; i += 1
        elif a == "--verbose": opt["verbose"] = True; i += 1
        else: raise SystemExit(f"unknown option {a}")
    md.CASE = opt["case"]; md.MODEL_FIRST = opt["first"]; md.MODEL_FRAME0 = opt["first"] - 2; md.LIFT = opt["lift"]
    ...
    n, mism, grab = md.compare(core, model, first_step, last_step, opt["verbose"])
    print(f"compared {n} frames from record {root} (QuickNES frame {md.MODEL_FRAME0 + first_step}); mismatches: {len(mism)}")
