...
| **L2** | **1-2's eight frames** (H22 / H46-dynamic) — the clip's *entry* costs the speed (F246); the traversal is free. A sink entry that keeps the speed (F244) is priced at ~13 frames of overshoot against a deficit of 8. | 8 | 60 | **CLOSED 2026-08-26 (F283) — by arithmetic, not by the dry.** The level's **ceiling is 3 frames** (exhaustive rung, layer 77 vs the WR's 80; Maru370's hand-made run independently lands on the same 3) against a **requirement of 8**, and F145 proves every other part of 1-2 already optimal, so nothing can supply the other 5. **3 < 8** ⇒ no route through 1-2 crosses its framerule, and its banked frames are provably worthless. L11 (the scroll-refund residue) was queued and closed unstarted on this. Superseded reading —  F144/F145 locate 3 real, core-verified frames in the wall clip that the **scroll refunds** (1-2's warp arms on `ScreenLeft` 2816, a function of where Mario has *been*), and the named residue — optimise `ScreenLeft` *through* the clip instead of gating on position and checking scroll after — has **never been run**. It is also the only place Maru's 3 frames can be (F145). Queued as **L11**. Original entry: Three 6 h archives, ~680M simulated frames, three roots x two subpixel granularities (`runs/E7-w12/{body,sub16,sub32}.log`): every one finished **exactly at the control** (`best=3764` vs baseline 3763), not one banked frame. E11's isolated frame came back empty too. Rollout-policy negative, not proof |
...
| **L4** | **8-4 room 2's exit pipe** — 15 frames of measured geometric loss at cols 150–152 (F245): an approach *arc* onto the pipe mouth (speed 38→23 against the c152/c153 block stack), never searched with a subpixel cell key. MrWint's 40-frame segment optimum does **not** cover it — it fixes the state at x 2373, which is exactly what the approach chooses (H39's seam corollary). Goal = the record quantity with the monotonicity argument written down, and the wrong-pipe trap observed in the control (F267). (Its sibling, 4-2's warp-zone drop, is 30 frames but 4-2 needs 13 and is measured closed on both routes.) | 1 | 15 | **DRY 2026-08-26 (F284).** Both roots ran their full 6 h at 150,000 cells: `a` 221.47M frames / 144 goals, `w` 160.42M frames / 147 goals — **~382M frames and 291 page-9 pipe entries, not one earlier than the WR's 16182**. `maxx=2572` against the WR's entry x of 2436, so the frontier reached well past the pipe and simply never got there sooner. The 15 priced frames are not reachable by an arc search from either root. Readable only because the goal was position-qualified mid-session (F274) — the bare `GES==3` had been poisoned at 16070 by the loop-back pipe. Superseded reading: both roots at full size (150,000 cells, 6 h): `a` (approach) and `w` (whole room, never run before s22). Control green at `GOAL frame=16182 +0`. **Its goal was fixed mid-session (F274):** room 2's loop-back pipe satisfied a bare `GES==3` 112 frames "early" and, via the improve-only incumbent rule, had made every real candidate unreportable; the goal now requires page 9. Project-wide audit (F276) found no other run affected |
...
|---|---|---|---|
| **8-4** | **any 1 frame** | 38 (room 3, L3 parked) + 15 (room 2 pipe, **L4 dry F284**) | **the only 1:1 level — L3 and L4 both live here.** Its 5 lag frames are NOT available (F264) |
| 8-2 | 19 | **114** | L1 running |
| ~~1-2~~ | 8 | ceiling **3** | **CLOSED (F283)** — 3 < 8, by counting |
