...
| **L3** | **8-4 room 3's approach** (H25) — one frame, in the only unquantized level. The room reduces to one question (F133c): the WR crosses x >= 3457 into a class paying **34** and the floor is **33**, so *is a 33-cost end class reachable at step <= 161?* **The board previously called this un-run; it is not — it was run twice (F133 d/e) and both went dry. But F272 shows both used a beam key that cannot represent the answer:** the key had none of `x_spd_abs`, `moving_dir`, `facing_dir`, `is_on_ground`, `running_speed`, and the R=33 classes are exactly a **landing frame** (ground, running, abs 0, facing LEFT, 4.8-11 px/frame) that an h-ranked beam discards for the faster state in the same speed band. | 1 | 38 | **RUNNING**, Linux — the approach re-run with the new `cls` axis (`runs/L3-w84r3/`, `docs/experiments/L3-w84r3-classkey.md`); phase 2 is the exhaustive continuation from step 162 |
