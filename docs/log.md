...
## 2026-09-01 (later) — Mac session, box online part-way: record-attempt re-check with a fresh model

**Ask.** "Another check to see about beating the L+R TAS … your model is new … look for new shit, don't bring up old shit; if there's flaws in something, that is a good thread." RTA-1 explicitly out of scope. No experiments or searches; the box came online mid-session and was used only to read the existing WR dump.

**Method.** Read PROCESS, STATUS, open-threads, hypotheses, facts, strategy-review, timing/warp/input/oob models and the E10/H12/H49/H50/P4E write-ups end to end, then asked what every closure rests on. Two pillars: the 40-subpixel cap under every x bound (H47) and the framerule arithmetic. Re-derived the framerule levers and found each rigid for a reason already in the ledger — boot phase fixed from power-on (cold boot clears $077F, boot lag fixed, Start rows 34–43 all give control at 197, F31); lag delays everything uniformly, no relative shift; `TimerControl` is the only relative shifter and loses (F255); the countdown is 1:1 and needs a `GameTimerDisplay` write that does not exist; the card is forced by warp mode 0 and by flag→`NextArea`; 8-4's 96-frame rise is forced by castle `AreaType` (H49). So the framerule side had nothing new. Went after H47 instead, because "un-enumerated" is a statement about work not yet done.

**Found — and then closed on the dump.** `PositionPlayerOnHPlat` (smbdis 10932) adds a horizontal platform's per-frame displacement to `Player_X_Position` in the enemy loop, after the player's own physics — run + carry, a genuine over-cap writer that still reads as `Player_X_Speed` 40. The project knew horizontal platforms 'would move x' (omissions table; F227 'relevant to 8-x, not here') but never enumerated 8-x. `tools/area_data.py E_CastleArea6` gives **page 4 col 6 id $28 = `XMovingPlatform`, in 8-4 room 1, over the 144-px bottomless pit at cols 66–74** — the unquantized level. Then the WR dump (`ram_trace`, slot 4, rows 15598–15720) killed it: the platform spawns when Mario first reaches x ≈ 929 (F64's `ScreenLeft` gate at rel 112, F229's ceiling), always starts moving LEFT (`MoveWithXMCntrs` negates the counter while the primary's d1 is clear) and stays left for 120 frames; a cap-speed crossing ends by spawn + 104; HappyLee touches it for one frame (15667) at a zero-integer-delta frame and loses nothing. Riding it subtracts; catching the rightward phase costs ≥ 16 (really 30+) frames for ≤ 17 px. **F295.** Then finished the enumeration: every writer of `Player_X_Position`/`Player_PageLoc` in the ROM — nine sites — each classified; the vine jump-off has no climbed route vine, the whirlpool has no route `Hole_Water` (checked `L_WaterArea3`), the loopback is −4 pages, the rest were already known. **H47's player-position class is enumerated and inert (F294); H47 → parked.** Not a refutation of H47 as a category (a write primitive would reopen it, and E10 closed that surface), but the premise behind every closure is now a checked list rather than an assumption.

**Also considered, not advanced:** over-cap via stomps/shells/Bullet Bills (y-only or shell-only); the swim cap (F271 holds — every area entry clears speed, and 8-4's water is unavoidable by topology); `Platform_X_Scroll` for `ScreenLeft`-gated goals (1-2/4-2 carry only vertical lifts); the ending coast (F223 + L7 r5 already negative); the 1-2 arithmetic (3 < 8 holds whether or not the refund is kept). **Live, needing the box (listed in STATUS):** Maru's 8-4 room by room against F265 (his 14 slower frames are all in 8-4, but F11 is a room-3 claim); the 4-2 warp-key minimum with `--beam-buckets --goal-offset`; H42's x-1981 seam (needs the coupled end-game term).

**Files.** F294/F295 in `docs/facts.md`; H47 status and evidence in `docs/hypotheses.md`; STATUS stamp + the omissions row; this entry. No tools changed. `tools/rta_line_build.py` from the RTA-1 session is still untracked — not this unit's file, left for that track to commit.
