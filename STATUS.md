...
| P2.3c-11 | **Free soundness validation of the bucketed beam: re-run the 4-2 chain's seven seams and reproduce 553.** The `s4*_layers` dirs are retained under `runs/P2.3c-2c/`. Harvest bucket representatives from each retained final layer (runbook §3.5) and re-chain with `--beam-buckets`. **The value is NOT the frames it might find in 4-2** — one frame there is worth zero (quantized, 21-frame gap, though it does hand the warp key budget). **The value is that 4-2 is the validation case with a known answer:** 553 was certified "every segment at its movement bound" across seven single-scalar seams, so a sub-553 result is direct evidence that seams have been leaking frames everywhere — including **8-4, where one frame IS the record** (H24). Reproducing 553 validates the machinery before 8-4 leans on it; beating it reframes every closed level. Also the natural place to test H41. | A/infra | S | — | Either 553 reproduced (machinery validated) or a sub-553 chain, core-verified; plus an H41 verdict and an explicit statement of what a sub-553 implies for 8-4 |
