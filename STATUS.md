...
## Running jobs
- **P2.1b-root1229-v3** (Linux box, started 2026-08-21 ~19:55): `build/bfs_par` v3 (compact states) pole search from the WR's frame-1229 state, 10 workers, 7 GB. Check `tail runs/P2.1b-root1229-v3/stdout.log` and `terminals.txt` (see its README). The v2 run hit its 1.52M-state cap at layer 14 (frame 1243), no terminal.

## In progress
- **P1.2-lite** — build MrWint/smb-opt + validate vs QuickNES. Started 2026-08-21. Checkpoint: built (rustup nightly-2018-06-01, `~/.cargo/bin`); `trace11pipe` mode added; model exact on the WR's third room with the goomba bounce injected (F53, `docs/experiments/P1.2-lite-smb-opt.md`). Remaining: (a) decide model extension (goombas) vs hybrid; (b) expose XPosHeuristic tables for `bfs_par`; (c) run smb-opt's own W11 cases to reproduce its 174/59-frame results.
- **P2.1b** — 1-1 third room relaxations on the current core. Started 2026-08-21. Part (3) first: `build/bfs` from the WR state at frame 1229 (stairs top, x 3033) with `--deadline 1284 --target-x 3158` (H27 test); then (1) the FPG trigger condition from the code; then (2) the flat-world frontier. Checkpoint: v1 hit the cap at layer 10; v2 (2 KiB states) hit 1.52M at layer 14 (`runs/P2.1b-root1229-par/stdout.log`: 16,127,847,3608,11616,30928,70096,132684,219936,354364,489820,709928,1087176,1518208+; ×1.4/layer); v3 (varying-address compact states, 184 addrs after 7 layers, verified identical state sets) launched as the running job above. FPG geometry written down (F51). Still to do: (2) flat-world frontier; widen roots to 1176/1200; write `docs/experiments/P2.1b-pole-search.md` results.
