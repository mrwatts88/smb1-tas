...
Updated: 2026-08-21 (session 3 end — P0.6, P1.1, P2.1a, P0.8 done; P2.1b + P1.2-lite in progress with a running job; next: check the job, then finish P2.1b)
...
## Running jobs
- **P2.1b-root1229-v4** (Linux box, pid in `runs/P2.1b-root1229-v4/stdout.log` via `pgrep -x bfs_par`; started 2026-08-21 ~20:35): `build/bfs_par` v3 engine with bounds (a)+(b)+(c) (F54), pole search from the WR's frame-1229 state, 10 workers, 7 GB. **Check first next session:** `tail runs/P2.1b-root1229-v4/stdout.log` — ends with `terminals:` (see `terminals.txt`; T_set ≤ 1813 = candidate 21-frame save → re-verify in FCEUX/BizHawk), `LAYER FULL` (inconclusive) or `no live states left` (proof: no grab ≤ 1284 from this root). Its README explains the bounds. If it is still running, leave it; nothing else needs the CPU.

## In progress
- **P1.2-lite** — build MrWint/smb-opt + validate vs QuickNES. Started 2026-08-21. Checkpoint: built (rustup nightly-2018-06-01, `~/.cargo/bin`, `cargo build --release` in `third_party/smb-opt`); modes added to its `main.rs`: `trace11pipe` (model trace, F53: exact on the WR's third room with the goomba bounce injected), `case NAME` (W11PipeSpeedup reproduced: 174 frames, 266 states, 20 s; W11Flag left unfinished — 59 frames per the source), `xpos-dump FILE` (exports the XPos table → `data/xpos_table_11.txt`, gitignored, used by `bfs_par --xpos-table`, F54). Remaining: (a) decide model extension (goombas: the WR bounces on one at row 1112) vs hybrid model/core; (b) write `docs/experiments/P1.2-lite-smb-opt.md` conclusions into STATUS Done when (a) is decided. The local smb-opt edits are saved as `tools/smb-opt-modes.patch`; `tools/build_core.sh` re-clones, applies it, builds and regenerates the XPos table.
- **P2.1b** — 1-1 third room relaxations on the current core. Started 2026-08-21. Part (3) first: `build/bfs` from the WR state at frame 1229 (stairs top, x 3033) with `--deadline 1284 --target-x 3158` (H27 test); then (1) the FPG trigger condition from the code; then (2) the flat-world frontier. Checkpoint: v1 hit the cap at layer 10; v2 hit 1.52M at layer 14 (no grab); v3 engine = compact states + bounds (a) speed, (b) XPos table, (c) descent (F54); run v4 from root 1229 is the running job above. Remaining: read v4's result; run the full third room from root 1040 (`--root 1040 --deadline 1284 --target-x 3158 --xpos-table data/xpos_table_11.txt --y-target 162`, expect ×1.8/layer growth early — may need the cloud box); widen roots (1176/1200); part (2) flat-world frontier not started; then write up and move P2.1b to Done. FPG geometry written down (F51). Still to do: (2) flat-world frontier; widen roots to 1176/1200; write `docs/experiments/P2.1b-pole-search.md` results.
...
| 1-1 pole search (root 1229) | v2: 1.52M states at frame 1243, no grab ≤ 1284; bounds (b)+(c) −40% states/layer | `runs/P2.1b-root1229-*/stdout.log` (V) |
