...
|---|---|---|---|---|---|
| P2.3c-9 | **F129 — constrain the wrong-warp goal, THEN ladder the top-route warp cost down (the go/no-go for 4-2).** Part A (mandatory, correctness): `goal_refused` (`main.rs`) tests only `screen_left16 >= 1217`; F129 shows the warp also needs the entry frame to advance Mario's integer x by **0** (`Player_X_Scroll` = 0 — the WR's is 0 for the whole descent, ours was 2 and the offset decayed 132 -> 107). The model already computes it (`emu.rs:388` `x_scroll`). Part B (the decision experiment): with the goal correct, run a **descending deadline ladder** with `--beam-buckets` (d86 -> d80 -> d76 -> d72) from the 509 root to measure the top route's **minimum** warp cost. Today's 43-frame key (596 total = 553 movement + 43) was *found*, never minimised; the 575 line allows **22**. | A | M | — | Part A: beam-off gate byte-identical, and a GOAL whose core replay shows AreaPointer $2F **and** re-entry at x 24 on ScreenLeft page 0 (runbook §4.3). Part B: a measured minimum top-route key cost vs the 22 the line allows — the number that decides whether 4-2 stays open |
...
| P2.2a-port | **Unblock the `w84enemies` port with F130 (the 8-4 primary track's gate).** F126 deferred the port as "materially bigger than budgeted" because the cheep frenzy reads `Player_X_Speed`/`Player_X_Position`. F130 (verified in the disassembly) shows speed enters as a **3-valued class** and position is an **additive, player-relative** spawn base — not a branch. So the frenzy is frame-indexed up to a 3-class: port cost ≈ 3×, not 2^k. Port the plant class from `w42enemies` + the frenzy with the F130 key (`FrenzyEnemyTimer` phase, speed-class, slot occupancy), difftest to 0 incl. cheep stomps. | A | M | — | Difftest 0 diffs over a random battery incl. cheep stomps (the 9/100 battery that failed in P2.2a now passes); then the d194 rung is unblocked |
| P2.3c-11 | **Free soundness validation of the bucketed beam: re-run the 4-2 chain's seven seams and reproduce 553.** The `s4*_layers` dirs are retained under `runs/P2.3c-2c/`. Harvest bucket representatives from each retained final layer (runbook §3.5) and re-chain with `--beam-buckets`. **Reproducing 553 proves the machinery sound; coming in under 553 proves single-scalar seams were silently costing frames on a level we had called closed** — and every frame under 553 hands the warp key more of the 22-frame budget. Honest upside by current accounting: ~1 frame (the 552-vs-553 wrap-up note). Also the natural place to test H41 (time as a hard constraint, exit-state quality as the score). | A/infra | S | — | Either 553 reproduced (machinery validated) or a sub-553 chain, core-verified; plus an H41 verdict |
...
- **Does 4-2 reclaim priority from 8-4?** `docs/decisions.md` (session 14 evening) made the 8-4
  ...
  alive" into a number), then return to the 8-4 queue via P2.2a-port.
