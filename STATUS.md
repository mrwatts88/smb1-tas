...
Updated: 2026-08-21 (session 4 — P2.1b-m done: the validated player model answers the pole-approach question exhaustively (H27 refuted from the stairs top); P1.2-lite validation done; next: goombas in the model, then the whole third room from the fixed entry)
...
## Running jobs
- **root1045 probe** (Linux box, `pgrep -x smb-opt`, started 2026-08-21 ~21:40, `ulimit -v 9.5 GB` so it dies cleanly): `smb-opt bfs11 data/wr/wr_inputs.bin 1048 0 239 --root-frame 1045` (exact model, enemy-free, whole third room, deadline 1284). Check `runs/P2.1b-model/root1045_d239.log` (layer lines; expected to die at the memory cap — record the last layer reached in `docs/experiments/P2.1b-pole-search.md`; a grab ≤ 1284 would be a candidate only if its path avoids the goombas, F58). Nothing depends on it.
- (none else.) (`P2.1b-root1229-v4` was killed by the OOM killer at layer 18 — 1.94M states, 500 s/layer — and is superseded by the model-side search, F57. Do not restart it.)

## In progress
- **P2.1b** — 1-1 third room. Parts done: (1) FPG trigger (F51) + the slide law (F55: only a glitch grab is fast); (3) H27 refuted from the WR's stairs-top state by the model-side BFS (F57, `docs/experiments/P2.1b-pole-search.md`). Part (2) flat-world frontier is now answered by MrWint's exact x-table: x ≥ 3158 is unreachable before frame 1278 from the fixed entry (WR: 1280). Remaining = P2.1b-m2 below (goombas in the model + the whole room from frame 1045); when it is done, move P2.1b to Done.
- **P1.2-lite** — smb-opt built, validated exact on 82k random frames after two fixes (F56), modes `trace11pipe`/`bfs11`/`xpos-dump`/`case` in `tools/smb-opt-modes.patch` (`tools/build_core.sh` applies it). Remaining = the enemy question, which is P2.1b-m2; then move to Done.

## Next up (ordered — the top unblocked item is the next unit of work)

| ID | Title | Track | Size | Depends on | Acceptance |
|---|---|---|---|---|---|
| P2.1b-m2 | **Goombas in the model + whole-room search.** Extend smb-opt's 1-1 model with the two third-room goombas as frame-indexed obstacles (F58/F59: positions are functions of the frame; per-layer state adds each goomba's alive/defeated flag, `Enemy_CollisionBits` d0, the 1-frame `StompTimer`; collision only on even frames; boxes from `BoundBoxCtrlData`; stomp iff y speed > 0 else death; enable `WithScrollPos` or verify that screen-relative boxes never wrap near Mario). Validate with `tools/model_difftest.py --root-record 1048` (random inputs from the first control frame: deaths and bounces must match the core, incl. the WR's bounce at 1112 without injection). Then `bfs11` from frame 1045 (prefix 0, 239 steps, per-layer dedup while a goomba is alive, global after) with the FPG goal and deadline 1284 — the first single search of the room from the fixed entry. Memory: run under `ulimit -v`, one search at a time (15 GB box; the compact engine needs ~50 B/state). | A | M | P1.2-lite | Difftest from 1048 exact incl. enemy events; search record (layer counts, grabs) in `docs/experiments/P2.1b-pole-search.md`; any grab ≤ 1284 replayed on the core with `build/bfs --replay-check`-style evaluation |
...
| Model vs core differential test | 0 differences on 82,524 frames / 1,600 trials (roots 1150/1230), 596 grabs, 71 FPG | `tools/model_difftest.py --n 200 --len 120 --seed 1` etc. (V) |
