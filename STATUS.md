...
**Updated:** 2026-09-01 (later; Mac, box over ssh) — **RECORD-ATTEMPT RE-CHECK with a fresh model** (user: new ideas or flaws, not a rehash; RTA-1 untouched). Deliverable was an assessment; two things were worth writing down. **(1) H47's player-position class is now ENUMERATED (F294):** nine writers of `Player_X_Position`/`Player_PageLoc` in the whole ROM, every forward one inert on the route — the premise under every x-bound closure is a checked list now. **(2) The route DOES contain a real over-cap primitive nobody had listed: 8-4 room 1's horizontal `XMovingPlatform` ($28) over the 144-px pit.** `PositionPlayerOnHPlat` adds its motion on top of Mario's run, invisible to the `Player_X_Speed` cap survey (F67/F225) — and it is phase-locked to move LEFT for the first 120 frames after its scroll-gated spawn, while a cap-speed crossing is over by spawn + 104; HappyLee touches it for one frame at a zero-delta frame, which is optimal (F295). The one crack found closes on inspection; the closure board is unchanged. **Two record-attempt follow-ups that are cheap now the box is back (neither started):** (a) **Maru's 8-4 room by room** — his 14 slower frames are all in 8-4 (RTA-1 §1), but F11's 'stop one frame earlier' is a *room-3* claim; replay `data/wr/maru_inputs.bin` on the core and read his five sub-area loads against F265 — any room faster than the WR's reopens that room; (b) **the 4-2 warp-key minimum** (STATUS 'The 4-2 hope') — `bfscx W42Main … --goal-offset 132 --beam-buckets off,y,spd,sub` from the col-30 arrival, the measurement F123 never had. `docs/log.md` 2026-09-01 (later) lists the levers re-derived and found rigid so they are not re-walked.
...
framerule**. **User decided: no framerule trade** (the runner is going for the record; the failing contact is the upper pair (50,3), the pipe wall jump is his second mint and he lands it). **Unit 2 done (F291, RTA-1 §3c):** the (50,3) mint built on the core is the *most forgiving* face — 3-frame takeoff (x 755–760), ~7-frame Right return, no subpixel sensitivity — and its invisible failures are **Left held into the jump (+0), Right back too early (+8), late takeoff (+9)**; only the no-tap case (+4) is visible. Rule set written. (a) is settled by the user's observation: the runner also bounces off the second goomba, lands on the run top inside the constriction and jumps from there — the same prefix shape as Maru's rows 6893–6898, so the takeoff-x windows apply directly. **Unit 3 (F292, RTA-1 §3d) — user's idea, and it works: running off the brick-run ledge into the lower pair's face mints +10 with NO jump** — tap Left in the last 3 ground frames, let go 2–4 frames, Right; Maru's hop exists precisely to avoid that contact. It would replace the (50,3) jump entirely (line: floor → under both groups → pit → pipes → the pipe wall jump he already lands). **Unit 4 (F293, RTA-1 §3e) — user's follow-up: rejoin the runner's path from the ledge mint.** Works: a re-jump from x 812–820 (5-frame window, just as Mario comes out from under the lower pair) lands cleanly on the three group's top at x 891–901, no contact, then his existing path. **Hand-crafted cost ≈ 2 frames vs the (50,3) line** (on the group 7 frames later, 12 px further, same speed). **SESSION END 2026-09-01 — where to go from here (three options, none started):** (1) **The 2 frames** — engine-optimise the ledge line's continuation (post-mint state → the three group's top / x ≥ 1005; the tap position, release count and re-jump frame were grid-picked, not optimised). (2) **The runner's slack** — a 60 fps frame count of one of his 4-2s (main-area start → 8-1 card, against the F28 boundary); decides whether 2 frames fit at all. (3) **His pipe section** — from the three group to the warp pipe his route differs from Maru's (he lands on pipe corners and wall-jumps the warp pipe; we have no inputs). A full "his line + our change" movie needs his inputs reconstructed from video (positions per frame → crafted inputs, verified on the core) — laborious but possible; alternatively hand him the ledge-segment fm2/GIF and let him integrate it, since everything after the three group is his own route. **Telling him is community outreach — the user's explicit call (docs/decisions.md rule), not to be done from a session.** Deliverables so far: `runs/RTA-1/ledge_line.{fm2,gif,mp4}`, `ledge_sheet.png` (regenerable, RTA-1 §3e), the rule sets in RTA-1 §3c/§3d.

**Needs from the user: the runner's slack** — a 60 fps frame count of his 4-2 (main-area start → 8-1 card vs the F28 boundary) or any input file — 2 frames fit only if he has ≥ 2. **Next unit (engine, on the box):** optimal continuation from the post-ledge-mint state to the three group's top / x ≥ 1005 (`bfscx W42Main`, `--lift`, enemies off) to see how much of the 2 frames is recoverable. Then: (b) build the pipe wall jump on the core and map its windows the same way (his second mint — he lands it, but its +9/+10 split is the other half of the zero margin); (c) turn §3c into a one-page route card with position cues. Superseded question, kept for the record: *is "easy" worth a framerule?* *No* → consistency
...
**ORDER — REWRITTEN session 22. Read this, not the session-19 order below it.** The board is down to
**three live threads** (`docs/open-threads.md`): 8-2 (running), 8-4 room 2's pipe (running), and the 8-4
novelty sweep (running). The session-19 order is **stale and will send you to re-run finished work**:
...
~~0. L11 — 1-2's scroll refund.~~ **Added and then CLOSED within the same session (F283).** Queued when
   the user asked what could still be done about the 1-2 clip; closed an hour later when following the
   Maru arithmetic to its end showed the level is shut by **counting, not by searching**: 1-2's ceiling
   is **3** frames (exhaustive rung, and Maru's hand-made run independently lands on the same 3) against
   a requirement of **8**, and F145 proves nothing else in the level can supply the other 5. Beating the
   scroll refund would recover frames that cannot pay. **Do not re-open it looking for the other 5 —
   F145 is the proof there are none.**
...
Everything below those is older Track A ladder work, unchanged in priority relative to each other.
...
|---|---|---|---|---|---|
| ~~L11~~ | **CLOSED UNSTARTED 2026-08-26 s22 (F283) — 1-2's ceiling is 3 frames against a requirement of 8, so beating the scroll refund would recover frames that still cannot pay. User's call, and the right one.** ~~1-2's wall clip: bank the 3 frames the scroll is refunding — the one named, never-run experiment on this route, and the only place Maru's 3 frames can be.** **The clip is not costing time; it is already GAINING 3 frames** (F144, core-verified 77 vs 80 with 0 mismatches, and the arrival strictly *dominates* the WR's: x 0xae130 vs 0xae0f0, x_spd 0x28fc vs 0x28d8, identical y/state). **The scroll takes them back:** `ScrollLockObject_Warp` (smbdis.asm:3566) arms `WarpZoneControl = 4` when **`ScreenLeft_X_Pos` reaches 2816** — a function of where Mario has *been*, not where he *is*. At step 1217 we sit at the WR's step-1220 x (2935) with more speed (40 vs 22) and more height (Y 107 vs 124), but `ScreenLeft` **2806 vs 2809** — and from there the warp is dry at 60, exactly what the WR pays. **In 1-2 a frame banked before the warp zone is a frame lent to the scroll.** **The residue, stated exactly (F145):** 2,016,915 goal transitions reach the post-clip milestone at layer 77 and **only the auto-pick and its scroll-maximal parent were ever followed**. The experiment is a **goal-function change, not more search**: optimise `ScreenLeft` *through* the clip — make scroll the ranked quantity across the whole clip segment — instead of hitting a position gate and checking the scroll afterwards. `offset-census --sl` (ranks a layer by absolute ScreenLeft descending) already exists for this. **Why L2's dry does not cover it:** L2 was three `explore` rollout archives over the whole level goaled on `WorldNumber == 3`; this is an exhaustive/beamed pass over the clip segment alone with scroll as the ranking axis — a different instrument at a located site, not more of the same. **And F145 re-prices Maru's route onto exactly this spot:** his 3 frames are not in the intro, the opening, the speed-capped body, the pit, or anything after step 1113, so **the wall clip is the only place left for them** — and it is where this 3 was found. **F282 sharpens this into the strongest reason to run L11:** Maru is quoted at **3 frames faster than HappyLee** *and* **5 from the framerule** against HappyLee's **8**, and 8 - 3 = 5 **only works if those 3 frames survive to the level's end** — i.e. he is not refunded. So either the refund is beatable (and a human found the line with **no search tool at all**, making L11 a reproduction rather than a gamble), or F145's elimination is wrong, or the numbers are not commensurable. His route is **RTA-rules (no L+R)**, so it beats the L+R WR *without* the input the WR leans on. Settling it cleanly needs Maru's movie file, which we do not have. **Sanity check that does not change: even a perfect L11 leaves 1-2 five frames short of its framerule** — 3 banked frames, not a record. | A | M | — | Either a core-replayed 1-2 line reaching the warp earlier than core 3763 (recorded, then FCEUX + BizHawk), or a per-layer statement that no layer-77 arrival exists whose ScreenLeft carries to `2816` by the WR's own frame — which would close 1-2 with a real proof instead of a rollout dry |
...
| ~~P2.2a′~~ | **PARKED s22 — this IS H25/L3, and L3 executed exactly this design (a set of apex states, each with its own return cost) and came back dry from a corrected key. Do not re-open as a wider beam.** ~~8-4 room 3: the multi-apex seam, ENEMY-FREE FIRST.~~ F125 proved the return leg optimal *from the WR's apex* (h = 33 = WR, the 32-rung dry at layer 1), so H25's frame can only be in **reaching a different apex state** — which an approach search goaled on the WR's apex deletes by construction (H39's seam corollary). Search the 162-frame approach with **generic** buckets (mechanism unknown ⇒ discovery keys, H40) including an apex-band axis, emit a **set** of apex-region states, and compute the return cost for each. Run it **enemy-free** and let the core adjudicate the plants/cheeps per candidate (the F126 cheapest-order note, line 117): one core replay checks one path exactly, which is far cheaper than modelling the frenzy up front. | A | M | — | A set of ≥1 non-WR apex states with per-apex return costs; either a 32-frame return from one of them (= the H25 frame, then the record pipeline) or a dry across the whole set |
...
- 2026-08-26 s22 — **1-2 CLOSED BY ARITHMETIC (F283), and L11 closed unstarted with it.** The level's
  ...
  other part of the level already optimal. 3 < 8, so no route through 1-2 crosses its framerule and the 3
  banked frames are provably worthless. **This is a stronger closure than the L2 rollout dry it replaces:**
  counting, not "we searched and found nothing". The user's call to take the published community number at
  face value is what produced it — discounting it was keeping the level artificially open.
...
- 2026-08-25 s22 — **L3 / H25 DRY a third time, from a key that could have seen it** (F275,
  ...
  **bound-pruned to zero at layer 188** with no goal. The `cls` axis bit — layer counts differ from both
  ...
  so a genuinely different candidate set hits the same wall. **H25 is now PARKED by the user — not refuted, and
  deliberately NOT left as a "widen the beam" follow-on** ("I'm done widening beams. I'm done just searching
  further into the abyss.", s22). All three *structural* objections are closed — F125/H39's apex-goal defect
  (L3 emitted a set of 5,225), F272's key defect (the `cls` axis, verified to have bitten), and what remains is
  only beam **width**, which is not an idea. **Reopens only on a different primitive:** a *backward* reachability
  from the 1,280 already-enumerated R=33 end classes (`runs/L3-w84r3/endclasses.log`) to a step ≤ 161 state.
  Do not build that for one frame — but if it ever exists for another reason, H25 is free. 19 GB of layers deleted;
  `launch.sh` regenerates them in 27 min.
...
| moving platforms / lifts | 4-2 lift modeled + validated (F73/F81); **the route's only other lifts are 1-2's four `$26`/`$27` (vertical) and 8-4 room 1's `XMovingPlatform` `$28` (horizontal), the latter read out of the WR dump and closed (F295)** | 4-2, 1-2, 8-4 r1 | vertical lifts do not move x (F68); **the horizontal one IS an over-cap carry (`PositionPlayerOnHPlat`, run + carry, invisible to the cap survey) but is phase-locked LEFT for any cap-speed crossing — F295** |
