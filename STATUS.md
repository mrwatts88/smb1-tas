...
0. **L11 — 1-2's scroll refund.** Added s22 when the user asked what could still be done about the 1-2
   clip. It was never queued despite being named in F144/F145 as *the* residue of that level — prose,
   not a unit, which is the third time that failure has shown up in this session. It is the only
   experiment left anywhere that is a **goal-function change rather than more search**, and it is the
   only place Maru's 3 frames can be. Not blocked on RAM the way L8 is.
