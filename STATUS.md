...
framerule**. **User decided: no framerule trade** (the runner is going for the record; the failing contact is the upper pair (50,3), the pipe wall jump is his second mint and he lands it). **Unit 2 done (F291, RTA-1 §3c):** the (50,3) mint built on the core is the *most forgiving* face — 3-frame takeoff (x 755–760), ~7-frame Right return, no subpixel sensitivity — and its invisible failures are **Left held into the jump (+0), Right back too early (+8), late takeoff (+9)**; only the no-tap case (+4) is visible. Rule set written. **Next unit:** (a) confirm the windows on the runner's *own* approach to the run's end (Maru's prefix arrives from a goomba bounce; the window is in takeoff x so it should transfer, but verify with a straight-run prefix); (b) build the pipe wall jump on the core and map its windows the same way (his second mint — he lands it, but its +9/+10 split is the other half of the zero margin); (c) turn §3c into a one-page route card with position cues. Superseded question, kept for the record: *is "easy" worth a framerule?* *No* → consistency
